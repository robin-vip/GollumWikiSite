...
```
     ...
     getopt_long 会进行argv顺序的调整，但也可以通过设置optstring改变它的方式，这里有两种
     （6）如果optstring的第一个参数是'+'或者POSIXLY_CORRECT被设置，则getopt_long 在原argv的顺序上遇到第一个非选项就返回-1；
     （7）如果optstring的第一个参数是'-'，则会将所有的非选项当选项处理，并且返回1，用字符代码1表示该选项；
```
