...
粘贴从外部（当前vim会话之外）复制的多行文本内容时，需要考虑到行首自动缩进问题。
复制的文本行首就有空白字符，加上vim本身的自动缩进会导致最终粘贴后的文本行首缩进不正常。
可以通过命令":set paste"进入paste模式，在该模式下行首不再自动缩进
粘贴后可以通过命令":set nopaste"退出paste模式。
...
替换指定行区间匹配字符到行尾的内容
:7303,7327s/\[.*$/,/g
将7303和7327行之间'['字符到行尾的内容替换成','  '['是特殊字符，所以前面需要加上转义字符'\' '$'代表行尾
```
